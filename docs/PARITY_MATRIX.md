...
| Deployment Agent readiness and update | Gaia HTTPAPI helper reused; no dedicated module in the pinned Management or Gaia collections | Observation and package-binding roles; live preflight, acquire, observe, decide, package-bind, update-plan, reconcile, reacquire-plan, evidence, next-plan, and completion modules; offline completion role | Lease, inventory binding, acquisition, graph, parser, numeric decision, package binding, planning, reconciliation, evidence, completion, tamper rejection, and composition fixtures passing | Two-member read-only observation passed with the lab TLS exception; strict TLS and live update execution pending |
