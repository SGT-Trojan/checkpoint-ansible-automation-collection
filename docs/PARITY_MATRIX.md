...
|---|---|---|---|---|
| MDS, domain, cluster, member, and policy resolution | Vendor facts modules provide source records | `cp_automation_target_resolve` | 47 passing | Pending |
| Management API pagination completeness | Vendor modules expose `limit` and `offset` | `cp_automation_pages_merge` | 20 passing | Pending |
...
| Cluster and gateway readiness | Gaia HTTPAPI helper with fixed `run-script` operation | `cp_automation_readiness_acquire`, `cp_automation_cluster_observe`, `cp_automation_readiness_decide` | 64 passing | Pending |
...
| Deployment Agent readiness and update | Gaia HTTPAPI helper reused for fixed status; `cp_gaia_put_file` is text-only; Ansible SSH transports the binary package | Observation, package-binding, completion, controller-local staging, and lease-bound firewall transport roles; live preflight, acquire, observe, decide, package-bind, update-plan, execution-preflight, artifact-stage, package-transport, reconcile, reacquire-plan, evidence, next-plan, and completion modules | Lease, inventory binding, acquisition, graph, parser, numeric decision, package binding, planning, staging, hostile chunk transport, reconciliation, evidence, completion, tamper rejection, and composition fixtures passing | Two-member read-only observation passed with the lab TLS exception; strict TLS, firewall package transport, and update execution live tests pending |
