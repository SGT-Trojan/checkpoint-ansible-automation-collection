...
| Deployment Agent readiness and update | Gaia HTTPAPI helper reused; no dedicated module in the pinned Management or Gaia collections; `cp_gaia_put_file` is text-only | Observation, package-binding, completion, and controller-local staging roles; live preflight, acquire, observe, decide, package-bind, update-plan, execution-preflight, artifact-stage, reconcile, reacquire-plan, evidence, next-plan, and completion modules | Lease, inventory binding, acquisition, graph, parser, numeric decision, package binding, planning, staging, reconciliation, evidence, completion, tamper rejection, and composition fixtures passing | Two-member read-only observation passed with the lab TLS exception; strict TLS, firewall package transport, and live update execution pending |
