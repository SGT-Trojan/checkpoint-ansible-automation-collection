...
"""Fail-closed authorization for live read-only package-state observation."""
...
class PackageStateLivePreflightError(ValueError):
    """A live package-state request is not authorized."""
...
def _fail(category: str, message: str) -> None:
    raise PackageStateLivePreflightError(category, message)
...
def _utc_timestamp(value: object, field: str) -> datetime:
    ...
    try:
        ...
        raise PackageStateLivePreflightError(
            "LEASE_INVALID",
...
def require_direct_gaia_controller(path: str = CONTROLLER_PROXY_INVENTORY) -> None:
...
def authorize_package_state_readonly(
    lease: object,
    ...
    """Authorize one exact, short-lived package-state observation."""
    ...
    if not isinstance(lease, dict) or set(lease) != LEASE_KEYS:
        _fail("LEASE_INVALID", "lease fields do not match the package-state contract")
    lease_id = lease["lease_id"]
    if not isinstance(lease_id, str) or not LEASE_ID.fullmatch(lease_id):
        _fail("LEASE_INVALID", "lease_id must be a UUIDv4")
    if lease["operation"] != "package_state_observation":
        _fail(
            "MUTATION_BLOCKED",
            "lease operation is not package_state_observation",
        )
