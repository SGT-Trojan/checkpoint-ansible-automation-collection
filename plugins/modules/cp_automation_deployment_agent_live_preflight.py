...
DOCUMENTATION = r"""
---
module: cp_automation_deployment_agent_live_preflight
short_description: Authorize one live read-only Deployment Agent observation
version_added: "0.1.0"
description:
  - Validates a short Deployment Agent lease, exact two-member target set,
  ...
  lease:
    ...
    suboptions:
      ...
      expires_at:
        ...
        required: true
      operation:
        description: Must be C(deployment_agent_observation).
        type: str
        required: true
      execution_mode:
...
EXAMPLES = r"""
- name: Authorize immediately before a Deployment Agent request
...
from ansible_collections.sgt_trojan.checkpoint_automation.plugins.module_utils.deployment_agent_live_preflight import (
    DeploymentAgentLivePreflightError,
    authorize_deployment_agent_readonly,
...
def run_module() -> None:
    ...
    try:
        require_direct_gaia_controller()
        authorization = authorize_deployment_agent_readonly(
    ...
    except DeploymentAgentLivePreflightError as error:
        module.fail_json(
            msg=str(error),
