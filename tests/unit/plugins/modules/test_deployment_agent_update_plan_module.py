...
class DeploymentAgentUpdatePlanModuleTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        sys.modules[
            (
                "ansible_collections.sgt_trojan.checkpoint_automation.plugins."
                "module_utils.deployment_agent_update"
            )
        ] = import_module("plugins.module_utils.deployment_agent_update")
        ...
        cls.module = import_module(
            "plugins.modules.cp_automation_deployment_agent_update_plan"
        )
    ...
    def test_argument_spec_and_check_mode(self) -> None:
        ...
        self.assertEqual(
            ...
            {"package_binding", "target_states", "selected_target_id"},
        )

    def test_validation_failure_is_sanitized(self) -> None:
        error = self.module.DeploymentAgentUpdateError(
            "TARGET_INVALID", "target state is invalid"
        )
        with patch.object(
            self.module,
            "plan_deployment_agent_update",
        ...
        result = caught.exception.args[0]
        self.assertFalse(result["changed"])
        self.assertEqual(result["category"], "TARGET_INVALID")
        self.assertEqual(result["msg"], "target state is invalid")
