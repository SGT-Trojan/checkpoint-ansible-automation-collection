...
class DeploymentAgentReacquirePlanModuleTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        ] = import_module("plugins.module_utils.deployment_agent_reconcile")
        sys.modules[
            (
                "ansible_collections.sgt_trojan.checkpoint_automation.plugins."
                "module_utils.deployment_agent_reacquire"
            )
        ] = import_module("plugins.module_utils.deployment_agent_reacquire")
        ...
        cls.module = import_module(
            "plugins.modules.cp_automation_deployment_agent_reacquire_plan"
        )
    ...
    def test_argument_spec_and_check_mode(self) -> None:
        with patch.object(
            ...
            return_value={"reacquisition_plan_sha256": "a" * 64},
        ):
    ...
    def test_validation_failure_is_sanitized(self) -> None:
        error = self.module.DeploymentAgentReacquireError(
            "PLAN_INVALID", "update plan is invalid"
        )
        with patch.object(
            self.module,
            "plan_deployment_agent_reacquisition",
        ...
        result = caught.exception.args[0]
        self.assertFalse(result["changed"])
        self.assertEqual(result["category"], "PLAN_INVALID")
        self.assertEqual(result["msg"], "update plan is invalid")
