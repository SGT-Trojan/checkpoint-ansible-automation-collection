...
class FakeAnsibleModule:
    params = {"update_plan": {}, "target_states": []}
...
class DeploymentAgentReconcileModuleTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.module = import_module(
            "plugins.modules.cp_automation_deployment_agent_reconcile"
        )
    ...
    def test_argument_spec_and_check_mode(self) -> None:
        with patch.object(
            ...
            return_value={"reconciliation_sha256": "a" * 64},
        ):
        ...
        self.assertEqual(
            ...
            {"update_plan", "target_states"},
        )

    def test_validation_failure_is_sanitized(self) -> None:
        error = self.module.DeploymentAgentReconcileError(
            "PEER_DRIFT", "peer build changed"
        )
        with patch.object(
            self.module,
            "reconcile_deployment_agent_update",
        ...
        result = caught.exception.args[0]
        self.assertFalse(result["changed"])
        self.assertEqual(result["category"], "PEER_DRIFT")
        self.assertEqual(result["msg"], "peer build changed")
